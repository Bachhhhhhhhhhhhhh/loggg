...
export function EOQCalculator() {
  ...
  const result = useMemo(() => {
    ...
    for (let q = Math.round(eoq * 0.2); q <= Math.round(eoq * 2); q += Math.round(eoq * 0.1) || 1) {
      chartData.push({
        q,
  ...
  const tableData: EOQRow[] = [
    { q: result.eoq, ordering_cost: result.orderingCost, holding_cost: result.holdingCost, total_cost: result.totalCost },

...
import { tickerData } from "@/data/kpis";
...
export function MarketTicker() {
  ...
  return (
    <div className="relative overflow-hidden border-b border-slate-800/80 bg-slate-950/90 h-7 flex items-center">
      <div className="absolute left-0 z-10 flex items-center gap-1.5 px-3 h-full bg-gradient-to-r from-slate-950 via-slate-950 to-transparent">
        <span className="relative flex h-1.5 w-1.5">
          <span className="absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75 pulse-live" />
      ...
      <div className="flex ticker-animate whitespace-nowrap pl-20">
        {items.map((item, i) => {
